...
#define optional_CPP11_OR_GREATER                                              \
    (__cplusplus >= 201103L || optional_MSVC_LANG >= 201103L)
#define optional_CPP14_OR_GREATER                                              \
    (__cplusplus >= 201402L || optional_MSVC_LANG >= 201703L)
#define optional_CPP17_OR_GREATER                                              \
    (__cplusplus >= 201703L || optional_MSVC_LANG >= 201703L)
#define optional_CPP20_OR_GREATER                                              \
    (__cplusplus >= 202000L || optional_MSVC_LANG >= 202000L)
...
#define optional_HAVE_STD_OPTIONAL                                             \
    (optional_CPP17_OR_GREATER && optional_HAS_INCLUDE(<optional>))
...
template <std::size_t I> struct in_place_index_tag {};
...
inline in_place_t in_place_index(
    detail::in_place_index_tag<I> = detail::in_place_index_tag<I>()) {
    return in_place_t();
}
...
#define nonstd_lite_in_place_index_t(T)                                        \
    nonstd::in_place_t (&)(nonstd::detail::in_place_index_tag<I>)
...
template <typename T> union storage_t {
  private:
    ...
    storage_t(value_type &&v) { construct_value(std::move(v)); }
    ...
    value_type const *value_ptr() const { return as<value_type>(); }

    value_type *value_ptr() { return as<value_type>(); }

    value_type const &value() const optional_ref_qual { return *value_ptr(); }
    ...
    value_type &&value() optional_refref_qual { return *value_ptr(); }
    ...
    typedef struct {
        align_as_type
            data[1 + (sizeof(value_type) - 1) / sizeof(align_as_type)];
    } aligned_storage_t;
    aligned_storage_t data;
...
#undef optional_ALIGN_AS
    ...
    void *ptr() optional_noexcept { return &data; }

    void const *ptr() const optional_noexcept { return &data; }

    template <typename U> U *as() { return reinterpret_cast<U *>(ptr()); }
...
template <typename T> class optional {
  ...
  public:
...
#if optional_CPP11_OR_GREATER
    optional_constexpr14 optional(optional &&rhs) noexcept(
    ...
    optional_constexpr optional(value_type &&value)
        : has_value_(true), contained(std::move(value)) {}

    template <class... Args>
    optional_constexpr explicit optional(nonstd_lite_in_place_type_t(T),
                                         Args &&... args)
...
#endif
    {
        if (has_value() == true && rhs.has_value() == false)
            reset();
        else if (has_value() == false && rhs.has_value() == true)
            initialize(*rhs);
        else if (has_value() == true && rhs.has_value() == true)
            contained.value() = *rhs;
        return *this;
    }
    ...
    optional &operator=(optional &&rhs) noexcept {
        if (has_value() == true && rhs.has_value() == false)
            ...
            contained.value() = std::move(*rhs);
        return *this;
    }

    template <class U, typename = typename std::enable_if<std::is_same<
                           typename std::decay<U>::type, T>::value>::type>
    optional &operator=(U &&v) {
        if (has_value())
            contained.value() = std::forward<U>(v);
        else
            initialize(T(std::forward<U>(v)));
        return *this;
    }
...
#if optional_CPP11_OR_GREATER
        noexcept(std::is_nothrow_move_constructible<T>::value &&noexcept(
    ...
    operator*() const optional_refref_qual {
...
operator==(nullopt_t, optional<T> const &x) optional_noexcept {
    return (!x);
}
...
template <typename T>
inline optional_constexpr bool operator<(optional<T> const &,
                                         nullopt_t) optional_noexcept {
    return false;
}
...
template <typename T>
inline optional_constexpr bool
operator<=(nullopt_t, optional<T> const &) optional_noexcept {
    return true;
}
...
template <typename T> optional<T> make_optional(T const &v) {
    return optional<T>(v);
}
...
template <class T> struct hash<nonstd::optional<T>> {
  public:
    std::size_t
    operator()(nonstd::optional<T> const &v) const optional_noexcept {
