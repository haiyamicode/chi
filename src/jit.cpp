...
static const jit_nuint ARRAY_DATA_FIELD_OFFSET = offsetof(CxArray, data);
static const auto ARRAY_SIZE_FIELD_OFFSET = offsetof(CxArray, size);
static const auto ARRAY_CAPACITY_FIELD_OFFSET = offsetof(CxArray, capacity);
static const auto ARRAY_FLAGS_FIELD_OFFSET = offsetof(CxArray, flags);
...
static jit_type_t empty_params[] = {};
static auto empty_signature =
    jit_type_create_signature(jit_abi_cdecl, jit_type_void, empty_params, 0, 1);

static jit_type_t malloc_params[] = {jit_type_nuint};
static auto malloc_signature =
    jit_type_create_signature(jit_abi_cdecl, jit_type_nint, malloc_params, 1, 1);

static jit_type_t free_params[] = {jit_type_nint};
static auto free_signature =
    jit_type_create_signature(jit_abi_cdecl, jit_type_void, free_params, 1, 1);
...
static jit_type_t array_internal_fields[] = {jit_type_nint, jit_type_uint, jit_type_uint,
                                             jit_type_ubyte};
static auto array_internal_struct = jit_type_create_struct(array_internal_fields, 4, 1);
...
static auto string_set_data_signature =
    jit_type_create_signature(jit_abi_cdecl, jit_type_nint, string_set_data_params, 2, 1);

static jit_type_t string_concat_params[] = {jit_type_nint, string_internal_struct,
                                            string_internal_struct};
static auto string_concat_signature =
    jit_type_create_signature(jit_abi_cdecl, jit_type_void, string_concat_params, 3, 1);

static jit_type_t printf_params[] = {string_internal_struct, array_internal_struct};
static auto printf_signature =
    jit_type_create_signature(jit_abi_cdecl, jit_type_void, printf_params, 2, 1);

static jit_type_t debug_params[] = {string_internal_struct};
static auto debug_signature =
    jit_type_create_signature(jit_abi_cdecl, jit_type_void, debug_params, 1, 1);
...
CompilationContext::~CompilationContext() {
    for (int i = 0; i < types.size; i++) {
        jit_type_free(types[i]);
    }
}

Function::Function(jit_type_t signature, CompilationContext *_ctx, ast::Node *_node)
    : jit_function(_ctx->jit_ctx, signature), ctx(_ctx), node(_node) {
    if (node) {
...
jit_value Function::insn_call(Function *fn, jit_value_t *args, long num_args) {
    return jit_function::insn_call(fn->get_jit_name(), fn->raw(), fn->signature(), args,
                                   (uint32_t)num_args);
}

jit_value Function::get_null_constant() { return new_constant(jit_nint(NULL)); }
...
jit_type_t Compiler::_compile_type(ChiType *type) {
    ...
    case TypeKind::Fn: {
        ...
        if (fn.container) {
            params.add(add_type(jit_type_create_pointer(compile_type(fn.container), 1)));
        }
        ...
        return add_type(jit_type_create_signature(jit_abi_cdecl, return_type, params.items,
                                                  uint32_t(params.size), 1));
    }
    case TypeKind::Struct: {
        ...
        if (struct_.kind == ContainerKind::Union) {
            add_type(jit_type_create_union(fields.items, uint32_t(fields.size), 1));
        }
        return add_type(jit_type_create_struct(fields.items, uint32_t(fields.size), 1));
    }
...
void Compiler::compile_fn_body(Function *fn) {
    ...
    if (fn_def.is_built_in()) {
        ...
        case ast::BuiltinId::ArrayAdd: {
            ...
            auto value = compile_value_copy(fn, fn->get_param(1), array_type->get_elem());
            auto ret = compile_array_add(fn, fn->get_param(0), (jit_uint)array_.elem_size, value);
            fn->insn_return(ret);
            break;
        }
        case ast::BuiltinId::ArrayDelete: {
            ...
            jit_label no_free;
            auto flags = fn->insn_load_relative(addr, ARRAY_FLAGS_FIELD_OFFSET, jit_type_ubyte);
            fn->insn_branch_if(fn->insn_and(flags, fn->new_constant(ArrayFlag::Static)), no_free);
            compile_field_mem_free(fn, addr, 0);
            fn->insn_label(no_free);
            break;
        }
...
jit_value Compiler::compile_fn_call(Function *fn, ast::Node *fn_call) {
    auto &data = fn_call->data.fn_call_expr;
    ...
    bool is_method = fn_expr->type == ast::NodeType::DotExpr;
    if (fn_expr->type == ast::NodeType::Identifier) {
        auto &iden = fn_expr->data.identifier;
        fn_ref = iden.decl;
    } else if (is_method) {
        auto &dx = fn_expr->data.dot_expr;
        auto dot = compile_dot_expr(fn, fn_expr);
        assert(dot.method);
        fn_ref = *dot.method;
        auto ptr_type = get_resolver()->get_pointer_type(dot.ctn_type);
        args.add({dx.expr, ptr_type, dot.ctn_address});
        if (dot.vtable_fn) {
    ...
    for (size_t i = 0; i < param_last; i++) {
        ...
        auto ptype = fn_spec.params[i];
        auto value = compile_assignment_to_type(fn, arg, ptype);
        args.add({arg, ptype, value});
    }
    if (fn_spec.is_variadic && data.args.size > param_last) {
        auto arg = data.args[param_last];
        auto argc = data.args.size - param_last;
        auto va_type = fn_spec.params.last();
        auto elem_type = va_type->get_elem();
        auto elem_size = jit_type_get_size(compile_type(elem_type));
        auto va_tmp = fn->new_value(array_internal_struct);
        auto va_ptr = fn->insn_address_of(va_tmp);
        auto va_data = fn->insn_alloca(fn->new_constant(argc * elem_size));
        fn->insn_store_relative(va_ptr, 0, va_data);
        fn->insn_store_relative(va_ptr, ARRAY_SIZE_FIELD_OFFSET, fn->new_constant(argc));
        fn->insn_store_relative(va_ptr, ARRAY_CAPACITY_FIELD_OFFSET, fn->new_constant(argc));
        fn->insn_store_relative(va_ptr, ARRAY_FLAGS_FIELD_OFFSET,
                                fn->new_constant(ArrayFlag::Static));
        for (size_t i = param_last; i < data.args.size; i++) {
            auto value = compile_assignment_to_type(fn, data.args[i], elem_type);
            auto index = i - param_last;
            fn->insn_store_elem(va_data, fn->new_constant(index), value);
        }
        args.add({arg, va_type, va_tmp});
        va_list_ref = va_ptr;
    }
    array<jit_value_t> raw_args;
    for (auto &arg : args) {
        raw_args.add(arg.value.raw());
    }
    if (fn_ref_vtable) {
        return fn->insn_call_indirect_vtable(*fn_ref_vtable, compile_type_of(fn_ref),
                                             raw_args.items, uint32_t(raw_args.size));
    }
    auto fn_instance = get_fn(fn_ref);
    assert(fn_instance);
    auto value = fn->insn_call(fn_instance, raw_args.items, raw_args.size);
    auto a_start = (int)is_method;
    for (int i = a_start; i < args.size; i++) {
        auto &arg = args[i];
        if (should_destroy_for_type(arg.type)) {
            auto addr = fn->insn_address_of(arg.value);
            compile_destruction_for_type(fn, addr, arg.type);
        }
    }
    return value;
}
...
jit_value Compiler::compile_simple_value(Function *fn, ast::Node *expr) {
    ...
    case ast::NodeType::LiteralExpr: {
        ...
        case TokenType::STRING: {
            return compile_string_literal(fn, token->str);
        }
        default:
            panic("unhandled");
        }
    ...
    case ast::NodeType::PrefixExpr: {
        ...
        assert(data.prefix->type == TokenType::KW_DELETE);
        compile_mem_free(fn, compile_simple_value(fn, data.expr));
        return jit_value();
    }
    default:
...
jit_value Compiler::compile_assignment_to_type(Function *fn, ast::Node *expr, ChiType *dest_type) {
    auto src_value = compile_simple_value(fn, expr);
...
jit_value Compiler::compile_conversion(Function *fn, const jit_value &value, ChiType *from_type,
                                       ChiType *to_type) {
    switch (to_type->kind) {
    case TypeKind::Reference:
    case TypeKind::Pointer: {
        if (ChiTypeStruct::is_trait(to_type->get_elem())) {
            ...
            fn->insn_store_relative(addr, TRAIT_TYPE_FIELD_OFFSET,
                                    fn->new_constant(jit_nint((void *)impl)));
...
jit_value Compiler::create_string_constant(Function *fn, const char *str) {
    return jit_value_create_nint_constant(fn->raw(), str_lit_type, (jit_nint)str);
}
...
void Compiler::compile_block(Function *fn, ast::Node *parent, ast::Node *block) {
    ...
    if (vars.size) {
        for (long i = vars.size - 1; i >= 0; i--) {
            ...
            auto address = fn->insn_address_of(get_value(var));
...
void Compiler::_compile_struct(ast::Node *node, ChiType *struct_type) {
    ...
    // default constructor
    auto cons_signature = jit_type_create_signature(jit_abi_cdecl, jit_type_void, &this_, 1, 1);
    add_type(cons_signature);
    auto constructor = add_fn(cons_signature, nullptr);
    init_method_fn(constructor, "_new", struct_type, subtype);
    sdata->constructor = constructor;
    ...
    // default destructor
    auto des_signature = jit_type_create_signature(jit_abi_cdecl, jit_type_void, &this_, 1, 1);
    add_type(des_signature);
    auto destructor = add_fn(des_signature, nullptr);
    init_method_fn(destructor, "_delete", struct_type, subtype);
    sdata->destructor = destructor;
...
DotValue Compiler::compile_dot_expr(Function *fn, ast::Node *expr) {
    ...
    if (member->is_field()) {
        ...
        if (ChiTypeStruct::is_trait(dot.ctn_type)) {
            ...
            dot.ctn_address =
                fn->insn_load_relative(trait_ref, TRAIT_DATA_FIELD_OFFSET, jit_type_nint);
            auto impl_info =
                fn->insn_load_relative(trait_ref, TRAIT_TYPE_FIELD_OFFSET, jit_type_nint);
            auto method_index = fn->new_constant(jit_int(member->method_index));
            auto vtable =
                fn->insn_load_relative(impl_info, IMPL_VTABLE_FIELD_OFFSET, jit_type_nint);
...
void Compiler::compile_construction(Function *fn, jit_value_t dest, ChiType *type,
                                    ast::Node *expr) {
    if (type->kind == TypeKind::Array) {
        ...
        return fn->insn_store_relative(dest, opt.get_opt_flag_offset(),
    ...
    assert(struct_.data->constructor);
    fn->insn_call(struct_.data->constructor, &dest, 1);
...
void Compiler::compile_cprintf(Function *fn, const char *s) {
    auto v = create_string_constant(fn, s);
    jit_value_t args[] = {v.raw()};
...
void Compiler::compile_destruction_for_type(Function *fn, jit_value &address, ChiType *type) {
    ...
    case TypeKind::Struct: {
        auto struct_ = get_struct(type);
        jit_value_t args[] = {address.raw()};
        fn->insn_call(struct_.data->destructor, args, 1);
...
void Compiler::init_method_fn(Function *fn, const string &fn_name, ChiType *struct_type,
...
ImplInfo *Compiler::get_impl_info(TraitImpl *impl) {
    if (impl->itable_index < 0) {
        ...
        info->vtable_size = (int32_t)vtable->size;
        return info;
    }
...
void Compiler::compile_array_construction(Function *fn, const jit_value &dest) {
    ...
    static auto array_construct_signature =
        jit_type_create_signature(jit_abi_cdecl, jit_type_void, array_construct_params, 1, 1);
    jit_value_t args[] = {dest.raw()};
    fn->insn_call_native("_cx_array_construct", (void *)cx_array_construct,
                         array_construct_signature, args, 1);
}

void Compiler::compile_array_reserve(Function *fn, const jit_value &dest, jit_uint elem_size,
                                     const jit_value &size) {
    static jit_type_t array_reserve_params[] = {jit_type_nint, jit_type_uint, jit_type_uint};
    static auto array_reserve_signature =
        jit_type_create_signature(jit_abi_cdecl, jit_type_void, array_reserve_params, 3, 1);
    jit_value_t args[] = {dest.raw(), fn->new_constant(elem_size).raw(), size.raw()};
    fn->insn_call_native("_cx_array_reserve", (void *)cx_array_reserve, array_reserve_signature,
                         args, 3);
}
...
jit_value Compiler::compile_array_add(Function *fn, const jit_value &dest, jit_uint elem_size,
                                      const jit_value &value) {
    static jit_type_t array_add_params[] = {jit_type_nint, jit_type_uint};
    static auto array_add_signature =
        jit_type_create_signature(jit_abi_cdecl, jit_type_nint, array_add_params, 2, 1);
    jit_value_t args[] = {dest.raw(), fn->new_constant(elem_size).raw()};
    auto address =
        fn->insn_call_native("_cx_array_add", (void *)cx_array_add, array_add_signature, args, 2);
    fn->insn_store_relative(address, 0, value);
    return address;
}

void Compiler::compile_string_construction(Function *fn, const jit_value &dest,
                                           optional<jit_value> data) {
...
jit_value Compiler::compile_string_literal(Function *fn, string str) {
    auto s = add_string(str);
    auto p = create_string_constant(fn, s);
    auto temp = fn->new_value(string_internal_struct);
    auto addr = fn->insn_address_of(temp);
    fn->insn_store_relative(addr, STRING_DATA_FIELD_OFFSET, p);
    fn->insn_store_relative(addr, STRING_SIZE_FIELD_OFFSET, fn->new_constant(jit_int(str.size())));
    return temp;
}
...
jit_value Compiler::compile_string_concat(Function *fn, const jit_value &s1, const jit_value &s2) {
    ...
    fn->insn_call_native("_cx_string_concat", (void *)cx_string_concat, string_concat_signature,
                         args, 3);
    return temp;
}

jit_value Compiler::compile_arithmetic_op(Function *fn, ChiType *value_type, TokenType op_type,
...
jit_value Compiler::compile_constant_value(Function *fn, const ConstantValue &value,
                                           ChiType *type) {
...
jit_value Compiler::compile_array_loop(Function *fn, jit_value &address, cx::ChiType *type,
                                       ArrayOp op) {
...
void Compiler::compile_panic(Function *fn, const string &message) {
    ...
    static auto panic_signature =
        jit_type_create_signature(jit_abi_cdecl, jit_type_void, panic_params, 1, 1);
    auto s = add_string(message);
    jit_value_t args[] = {create_string_constant(fn, s).raw()};
...
Function *Compiler::compile_end_fn() {
    auto fn = add_fn(empty_signature, nullptr);
    fn->qualified_name = "__end";
    if (m_ctx->settings.enable_jit_dump) {
        jit_dump_function(stdout, fn->raw(), fn->get_jit_name());
    }
    return fn;
